...
from src.enemies  import Boss, SplitterEnemy, EnemyBullet, AllyGhost, AllyBullet, GhostEnemy
...
class Game:
    ...
    def __init__(self):
        ...
        self.paywall_screen = PaywallScreen(self.fonts)
        self.code_screen    = CodeEntryScreen(self.fonts)
        ...
        self._unlocked    = self._load_unlock()
        self._plays_count = self._load_plays_count()
        self._pending_diff = None   # difficulté choisie, en attente après paywall
    ...
    # ── Gestion essais / déblocage ────────────────────────────────────────

    def _load_plays_count(self):
        data = load_json(PLAYS_FILE, {})
        return data.get("plays", 0)

    def _save_plays_count(self):
        save_json(PLAYS_FILE, {"plays": self._plays_count})

    def _load_unlock(self):
        data = load_json(UNLOCK_FILE, {})
        return data.get("unlocked", False)

    def _save_unlock(self):
        save_json(UNLOCK_FILE, {"unlocked": True})

    def _can_play(self):
        """Retourne True si le joueur peut lancer une partie."""
        return self._unlocked or self._plays_count < MAX_FREE_PLAYS

    def _increment_plays(self):
        """Incrémente le compteur et le persiste."""
        self._plays_count += 1
        self._save_plays_count()
    ...
    def _init_session(self):
        self.player   = Player()
        self.enemies  = []
        self.allies   : list = []   # AllyGhost instances
        self.ally_bullets: list = []
        self.p_bullets: list = []
        self.e_bullets: list = []
        self.powerups : list = []
        self.fx       = EffectManager()
        self.wave_mgr = WaveManager(self.diff_speed)
    ...
    def _update(self, events):
        ...
        if self.state == self.S_MENU:
            # Push fresh leaderboard data to the menu screen each frame
            self.menu_screen.set_leaderboard(
                self.leaderboard.get_entries(),
                self.leaderboard.get_status(),
            )
            # Push trial state so the counter panel stays current
            self.menu_screen.set_trial_info(self._unlocked, self._plays_count)
            result = self.menu_screen.update(events)
            if result:
                action, param = result
                if action == "start":
                    if self._can_play():
                        # Essai gratuit ou déjà débloqué
                        self.diff_speed = param
                        self._increment_plays()
                        self._init_session()
                        self.state = self.S_PLAY
                        self.snd("wave_start")
                        self._start_bgm()
                    else:
                        # Essais épuisés → paywall
                        self._pending_diff = param
                        self.paywall_screen.start()
                        self.state = self.S_PAYWALL

        elif self.state == self.S_PAYWALL:
            result = self.paywall_screen.update(events)
            if result == "unlocked":
                # Paiement confirmé : débloquer définitivement
                self._unlocked = True
                self._save_unlock()
                self.diff_speed = self._pending_diff or 1.0
                ...
                self.state = self.S_MENU
            elif result == "code_entry":
                self.code_screen.reset()
                self.state = self.S_CODE_ENTRY

        elif self.state == self.S_CODE_ENTRY:
            result = self.code_screen.update(events)
            if result == "back":
                self.state = self.S_PAYWALL
            elif result == "unlocked":
                # Code validé — déblocage permanent
                self._unlocked = True
                self._save_unlock()
                self.diff_speed = self._pending_diff or 1.0
                self._pending_diff = None
            ...
            # Submit score once player has confirmed their name
            if (self.go_screen.name_done
                    and not self.go_screen.submitted
                    and self.score_sys.score > 0):
                name_str = self.go_screen.name_input.strip()
                if len(name_str) > 0:
            ...
            if result == "restart":
                ...
                if self._can_play():
                    ...
                    # Essais épuisés — ouvrir le paywall avec la même difficulté
                    self._pending_diff = self.diff_speed
                    self.paywall_screen.start()
                    self.state = self.S_PAYWALL
            elif result == "menu":
                self.go_screen.reset()
    ...
    def _update_play(self, events):
        ...
        # Allies (converted ghost companions)
        for ally in self.allies:
            ally.update(self.player.x, self.player.y)
            new_ally_bullets = ally.get_bullets(self.enemies)
            self.ally_bullets.extend(new_ally_bullets)
        self.allies = [a for a in self.allies if a.active]

        # Ally bullets — home toward nearest enemy
        for ab in self.ally_bullets:
            ...
            for e in self.enemies:
                if not e.active:
                    continue
                d = math.hypot(e.x - ab.x, e.y - ab.y)
                if d < best_d:
                    best_d, nearest_x, nearest_y = d, e.x, e.y
            ab.update(nearest_x, nearest_y)
        self.ally_bullets = [b for b in self.ally_bullets if b.active]
        ...
        if self.wave_mgr.game_finished:
            self._stop_bgm()
            self.go_screen.reset()
            self.go_screen.victory = True
    ...
    def _check_collisions(self):
        ...
        for e in list(self.enemies):
            ...
            if dist(px, py, e.x, e.y) < pr + e.radius - 4:
                if self.player.has_shield:
                    ...
                    self.snd("shield_hit")
                elif isinstance(e, GhostEnemy):
                    # Purple ghost touches player → convert to ally if player survives
                    player_killed = self.player.hit()
                    e.active = False
        ...
        # Balles ennemies ↔ joueur — ghost bullets may convert to allies
        for b in list(self.e_bullets):
            ...
            if dist(b.x, b.y, px, py) < b.radius + pr:
                ...
                # Purple homing bullet that didn't kill → convert to ally
                if (not player_hurt
                        and getattr(b, "homing", False)
                        and b.color == C_PURPLE):
                    self._spawn_ally(b.x, b.y)

        # Balles alliées ↔ ennemis
        for ab in list(self.ally_bullets):
            if not ab.active:
                continue
            for e in list(self.enemies):
                if not e.active:
                    continue
                if dist(ab.x, ab.y, e.x, e.y) < ab.radius + e.radius:
                    ab.active = False
                    destroyed = e.hit(ab.damage)
                    if destroyed:
                        self._on_enemy_killed(e)
                    else:
                        self.fx.sparks(e.x, e.y, C_PURPLE, 5)
                    break
    ...
    def _on_enemy_killed(self, e):
        ...
        # Chain reaction: explosive enemies damage nearby foes
        if getattr(e, "chain_explodes", False) and e.chain_radius > 0:
            self._trigger_chain_reaction(e)

    # ── ALLY GHOST ───────────────────────────────────────────────────────

    def _spawn_ally(self, x, y):
        """Convert a blocked ghost bullet into a 30s ally companion."""
        self.allies.append(AllyGhost(x, y))
        self.fx.explosion(x, y, C_PURPLE, 20)
        self.fx.screen_flash(C_PURPLE, alpha=40, decay=6)
        self.snd("collect")
        # Float text
        self.score_sys.float_text(x, y, "ALLIÉ !", C_PURPLE)

    # ── CHAIN REACTION ───────────────────────────────────────────────────

    def _trigger_chain_reaction(self, source, depth=0):
        """Explode source enemy, damaging all nearby enemies in radius.
        Depth prevents infinite recursive chains (max 3 hops)."""
        if depth > 3:
            return
        self.fx.chain_explosion(source.x, source.y, source.chain_radius)
        self.snd("explode_big")
        chain_killed = []
        for e in list(self.enemies):
            if not e.active or e is source:
                continue
            d = dist(source.x, source.y, e.x, e.y)
            if d < source.chain_radius + e.radius:
                # Damage proportional to distance: full damage at center, 1 at edge
                dmg = max(1, int(3 * (1 - d / (source.chain_radius + e.radius))))
                destroyed = e.hit(dmg)
                self.fx.sparks(e.x, e.y, e.color, 10)
                if destroyed:
                    chain_killed.append(e)
                    # Bonus score for chain kills
                    self.score_sys.add(
                        int(e.score_value * 1.5), e.x, e.y, self.player.combo,
                    )
                    self.player.add_kill()
        # Trigger secondary explosions for any newly-killed chain-exploders
        for e in chain_killed:
            self.fx.explosion(e.x, e.y, e.color, 20)
            if getattr(e, "chain_explodes", False) and e.chain_radius > 0:
                self._trigger_chain_reaction(e, depth + 1)
    ...
    def _draw(self):
        ...
        if self.state == self.S_MENU:
            ...
            self.menu_screen.draw(self.screen, self.stars, self.nebula)
            self.paywall_screen.draw(self.screen)
        elif self.state == self.S_CODE_ENTRY:
            self.menu_screen.draw(self.screen, self.stars, self.nebula)
            self.code_screen.draw(self.screen)
        else:
            self._draw_gameplay()
